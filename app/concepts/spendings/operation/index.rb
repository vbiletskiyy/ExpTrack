module Spendings::Operation
  class Index < Trailblazer::Operation
    ...
    def filter_exists?(ctx, filter:, current_user:, **)
      ctx[:filtered] = Spending.includes(:user_spendings).where(user_spendings: {user_id: current_user.id, sent_by: nil})
    ...
    def filter_by_category(ctx, filter:, user_spendings:, current_user:, **)
    ...
    def filter_by_amount(ctx, filter:, user_spendings:, current_user:, **)
      ctx[:filter_by_amount] = user_spendings.where(amount: filter)
    end

    def filter_by_description(ctx, filter:, user_spendings:, current_user:, **)
